...
import com.authsure.client.Version;
import com.authsure.client.exception.AuthSureAuthenticationFailedException;
import com.authsure.client.exception.AuthSureBadRequestException;
import com.authsure.client.exception.AuthSureException;
import com.authsure.client.exception.AuthSureForbiddenException;
import com.authsure.client.exception.AuthSureNotFoundException;
...
public class URLConnectionRestClient extends JacksonRestClient implements Serializable {
  ...
  public static final String USER_AGENT = "AuthSure Java Client " + Version.VERSION;
  ...
  protected IOException getError(IOException x, HttpURLConnection conn) throws IOException {
    if (conn != null) {
      ...
      switch (status) {
        case 401:
          return new AuthSureAuthenticationFailedException(error);
        case 404:
          return new AuthSureNotFoundException(error);
        case 400:
          if (error != null) {
            return new AuthSureBadRequestException("Received error: " + error,
                readCollection(error, List.class, AuthSureError.class));
          }
          throw new AuthSureBadRequestException();
        case 403:
          return new AuthSureForbiddenException(error); // TODO
        default:
          return new AuthSureException(error, x);
      }
    }
    return x;
  }
  ...
  protected HttpURLConnection setup(HttpMethod method, String url) throws IOException {
    ...
    conn.setRequestProperty("Accept", "application/vnd.authsure.v1+json"); // TODO Make constant
